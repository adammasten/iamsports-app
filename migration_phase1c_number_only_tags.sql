-- migration_phase1c_number_only_tags.sql — make player-tag labels correct for
-- NUMBER-ONLY roster spots. create_roster_placeholder already stores a nameless
-- spot as "#12" (name = '#' || jersey), so a coach can roster the whole team by
-- number (no names, no accounts) for tagging + stats. But the Phase 1b label
-- logic would then append the jersey again → "#12 #12". Fix: if the name is
-- already a "#N" sentinel, use it as-is; otherwise "First #N".
--
-- Label rule (all four sync points below):
--   name starts with '#'      -> the name as-is        ("#12")
--   real name + jersey        -> "First #N"            ("Conrad #12")
--   real name, no jersey      -> "First"               ("Conrad")
...
-- Backfill: re-apply the corrected rule to existing player tags.
update tags tg set name = case when split_part(p.name, ' ', 1) like '#%'
  then split_part(p.name, ' ', 1)
  else split_part(p.name, ' ', 1) || coalesce(' #' || nullif(trim(
    (select pt.jersey_number from player_teams pt where pt.player_id = tg.player_id and pt.team_id = tg.team_id limit 1)
