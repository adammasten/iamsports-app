...
**ORG LAYER — NEEDED, tracked as its own next-project (added July 8):** Adam confirmed a real "organization" tier is needed — a container ABOVE teams where 1-2 central org-admins control coaches across MANY teams (big-club / league case). Today the model is team-scoped (admin role is per-team, no org container over multiple teams). This does NOT block the current player-permission grid build (team-scoped permissions are true regardless of whether a team belongs to an org — the org layer sits ABOVE and gets built later without reworking the team-permission brick). Build order: finish the team-level permission foundation first, then add the org layer as its own project. FIRST STEP when starting it: read-only check whether any org/organization container already exists in the data (likely not — probably net-new: an `organizations` table + org-admin role + teams belonging to an org).

---

## PART 0 — Profile types (the account kinds)

Everything else hangs off "what kind of account is this." Five profile types (names locked July 8):

1. **Parent / Kid** — the account-holder unit. The parent operates it; the kid is the subject. These are ONE thing (a parent-kid profile is a single unit). Owns the kid's content; controls who follows the kid (invites Extended Family — see #4). Cannot view other kids' content unless it's public.

2. **Coach** — runs a team. Manages the team roster and the team permissions grid (Part A), controls tagging rights on team footage, posts to walls, deletes donated footage, gatekeeps join requests.

3. **Admin** — coexists with Coach but sits ABOVE it. Key power: **Admin can add/remove COACHES** (coaches manage players; admins manage coaches). NOTE: the admin-removes-coach mechanics don't need to be built immediately, but the model should reserve for it so it's not bolted on later.

4. **Extended Family** — grandparents, aunts, uncles — the wider circle beyond the household. (Name chosen deliberately: NOT "Family" or "Fan," because "Family" blurs with the parent account and "Extended Family" is instantly obvious to everyone.) A parent/kid account **invites** an Extended Family profile, attached to a specific kid. What they see (per Part D1): the kid's own public content + the PUBLIC team wall (not the private members-only team wall) + the kid's games (games sit behind the paid/membership tier). NEVER the private team wall, never other kids' private content.

5. **Editor** — the PAID TAGGER. A person who gets hired to tag/break down games. Has a profile (pays ~$5 to have it), and coaches/parents hire them; payment happens OUTSIDE the app. Feeds the AI-training-data moat (every tagged clip = labeled data).
   - **FLAGGED — design later, its own pass:** the Editor is fundamentally different from the other four — it's an OUTSIDE-THE-TRUST-CIRCLE role. An outside adult getting tagging access to a team's private minors' footage is a real child-safety question. The other four profiles live inside a family/team trust circle; the Editor reaches outside it. Capture it here, but design the Editor flow (hiring, access scoping, child-safety guardrails) as its own dedicated pass — do NOT lump it into the family/team permission build.

### CORE PRINCIPLE — Every relationship is removable (soft remove, preserve history)

Nothing is permanent. EVERY profile relationship can be removed, and the system must build removal in from the start — never a one-way door:
- **Parent/Kid** — removable from a team.
- **Coach** — removable by an admin.
- **Admin** — removable by another admin.
- **Extended Family** — removable by the parent who invited them.
- **Editor** — removable from whatever they were granted access to. (Especially important — this is the outside-the-trust-circle role; there must be a clean, instant "revoke this editor's access" action.)

**Design rule:** access is *granted*, and granting always implies *un-granting*. Anywhere the system lets someone add a person or flip a permission ON, it must also let them remove / flip OFF. No one-way doors — critical for a kids' app (if you can add an outside editor to your footage, you must be able to instantly kick them out).

**Removal is SOFT — cut future access, preserve past work/history.** Removing someone ends their access going forward but does NOT destroy the work or history they created: a removed coach's tagged games stay intact; a removed editor's tagging work remains (only their access ends); a removed family member's existence in past records isn't erased. Same "soft remove, preserve history" principle as the career vault (Part D2) and the Phase 0 cascade fix. Removal must never cascade-delete footage, tags, lineups, or game history.
