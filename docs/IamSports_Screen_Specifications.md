...
Walls include: a personal/public wall, a kid's (player's) wall, a team wall, and a coaches' board. Which walls a given user is allowed to post to is **role-gated** (see Roles below). One sharing system serves both videos and reels — it is not two separate mechanisms.
...
- **Sharing / "Pick a wall" plumbing** is the connective tissue for many screens (upload-share, reel-publish, player/team/coaches walls). It is the current build gap. Two known dependencies: the post-to-wall path needs to handle a target appropriately for reels (not only kid-centric), and reel content must resolve/render when published (so a published reel plays rather than showing "content unavailable").
