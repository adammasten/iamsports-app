...
## Post visibility — public vs. private (NEXT SLICE, not yet built)

**Status:** Designed, not built. Posting a reel to a kid's wall works today, but **everything posted is currently fully public** — there is no public/private choice yet. This section defines the missing piece.

**The two separate concepts (do not conflate):**
1. **Which wall** — where the content lives (kid's wall, team wall, coaches' board). *Built.*
2. **Visibility** — who can see what's on that wall. *Not built — this section.*

**The visibility model (the Facebook / private-Instagram mental model):**
- **Private** — only the kid's circle sees it: linked family + approved followers. Like a private Instagram account. Nobody outside the circle, no public link.
- **Public** — anyone with the link/URL can see it. Like a public profile.

**Rules:**
- **Visibility is a choice made at post time**, separate from picking the wall. Posting flow becomes: pick wall → choose public or private.
- **Default to PRIVATE for a kid's wall.** Public is a deliberate opt-in, never the default — child-safety posture (do not expose a minor publicly by accident).
- **This is the enforcement mechanism for the kid-wall rule already specced:** "the family controls public exposure of the kid; coach posts are forced private." A coach's post simply cannot be set to public; only the kid/linked parents can choose public.

**Technical reality (from investigation):** the current plumbing tangles visibility into the `audience` value — `audience='public'` means anyone can read; `audience='player'` is the family-only inbox. There is **no clean visible/private flag** on `post_to_wall` today. Building real visibility needs: a visibility choice in the UI, a way to represent private-on-a-wall (vs. the inbox), the **followers/approved-circle system** (who counts as "your circle" for private), the **share-link/URL system** (for public), and the RLS rules to enforce all of it. This is a meaty slice — own session.

**Open Questions:**
- Private = linked family only, or linked family + approved followers? (Depends on the followers system, which is itself unbuilt.)
- Is there a third tier (e.g. team-visible) or just public/private?
- How a public share link/URL is generated and revoked.
